...
public class Board {
    public static void main(String[] args) {
        ...
        System.out.println("Минимальное количество шагов = " + solver.moves());
        System.out.println();
    ...
    private int[][] blocks; //   поле (пустое место обозначим нулем)
    private int zeroX;    // координаты нуля
    private int zeroY;
    private int h; //  мера

    public Board(int[][] blocks) {
        int[][] blocks2 = deepCopy(blocks);   //   копируем, так как нужно быть уверенными в неизменяемости
        this.blocks = blocks2;
        h = 0;
        for (int i = 0; i < blocks.length; i++) {  //  определяем координаты нуля и вычисляем h(x)
    ...
    public Iterable<Board> neighbors() {  // все соседние позиции
        // меняем ноль с соседней клеткой, т.е.всего 4 варианта
